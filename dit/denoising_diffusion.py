...
class DenoisingDiffusion(nn.Module):
  ...
  def _denoise(self, sample, sigma, is_training):
    ...
    # apply scaling
    inputs = sample * self.parameterization.in_scaling(sigma).reshape(new_shape)
    ...
    skip = sample * self.parameterization.skip_scaling(sigma).reshape(new_shape)
    outputs = out * self.parameterization.out_scaling(sigma).reshape(new_shape)
